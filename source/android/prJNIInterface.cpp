...
void JNI_ShowScoreloopTerms()
{
    ...
    if (gJavaVM)
    {
        ...
        // Find the callBack method ID
        jmethodID method = env->GetStaticMethodID(cls, "ShowScoreloopTerms", "()V");
        if (!method)
        {
            prLog("Failed to get method ID\n");
            if (isAttached)
...
bool JNI_IsScoreloopEnabled()
{
    ...
    if (gJavaVM)
    {
        ...
        // Find the callBack method ID
        jmethodID method = env->GetStaticMethodID(cls, "IsEnabled", "()Z");
        if (!method)
        {
            prLog("Failed to get method ID\n");
            if (isAttached)
